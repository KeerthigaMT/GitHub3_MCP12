...
func Repos_disable_lfs_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/lfs%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_disable_lfs_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_lfs",
		mcp.WithDescription("Disable Git LFS for a repository"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_disable_lfs_for_repoHandler(cfg),
	}
}
