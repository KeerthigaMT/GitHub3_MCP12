...
func Actions_list_selected_repositories_required_workflowHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateActions_list_selected_repositories_required_workflowTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_actions_required_workflows_required_workflow_id_repositories",
		mcp.WithDescription("List selected repositories for a required workflow"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_selected_repositories_required_workflowHandler(cfg),
	}
}
