...
func Actions_list_required_workflow_runsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/required_workflows/%s/runs%s", cfg.BaseURL, owner, repo, required_workflow_id_for_repo, queryString)
...
func CreateActions_list_required_workflow_runsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_actions_required_workflows_required_workflow_id_for_repo_runs",
		mcp.WithDescription("List workflow runs for a required workflow"),
		...
		mcp.WithString("status", mcp.Description("Returns workflow runs with the check run `status` or `conclusion` that you specify. For example, a conclusion can be `success` or a status can be `in_progress`. Only GitHub can set a status of `waiting` or `requested`.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_list_required_workflow_runsHandler(cfg),
	}
}
